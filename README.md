...
Claude を活用した GitHub 自動化基盤(汎用)。任意の GitHub リポジトリから reusable workflow として呼び出して使う。
...
| アクター | 役割 |
|---|---|
| 人(リポジトリオーナー) | Issue 起票、優先順位付け、設計分岐のジャッジ |
...
利用者リポジトリの `.github/workflows/` に薄い shim を置いて、本リポジトリの reusable workflow を `uses:` で呼ぶ。

```yaml
# <consumer-repo>/.github/workflows/claude-impl.yml
name: Claude Impl
on:
  issues:
    types: [labeled]
jobs:
  impl:
    ...
    uses: <owner>/claude-automation/.github/workflows/reusable-impl.yml@v1
    secrets: inherit
```

`<owner>` には本リポジトリのオーナーアカウント / Organization を、`<consumer-repo>` には利用者リポジトリのパスを指定する。
...
5. 利用者リポジトリから reusable workflow を呼ぶ shim を追加
...
利用者リポジトリからは `@v1` で本リポジトリの reusable workflow を参照する想定(major version tag による安定参照)。
