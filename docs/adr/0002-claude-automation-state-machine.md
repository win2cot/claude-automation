...
- `anthropics/claude-code-action` 経由の `--max-turns N` は **ソフト締切**。num_turns が N を超えた時点で Claude が wrap-up モード(「自分の判断で完了させる」最終アシスタントメッセージ生成中)に入っていれば、turn 超過のまま `subtype: success / is_error: false` で着地する。tool_use ループ継続中なら強制終了 → `subtype: error_max_turns`(= `Impl-Aborted(reason=max-turns)` の A 経路)。

  観察根拠: 以下 4 件は全て `action SHA = 787c5a0c…` / `Claude Code v2.1.150` で完全同一(infra 差ではなく SDK/CLI 側の `--max-turns` 発火タイミングによる分岐)。

  | run | Issue | conclusion | subtype | num_turns | --max-turns |
  |---|---|---|---|---|---|
  | 26331824648 | #207 | success | `error_max_turns` | 41 | 40(効いた) |
  | 26369599009 | #86  | success | `error_max_turns` | 41 | 40(効いた) |
  | 26365419914 | #87  | success | `error_max_turns` | 41 | 40(効いた) |
  | 26410452890 | #142 | success | **`success`** | **48** | **40(効いていない)** |

  この仕様が明文化されていない場合、SM-04(max-turns 上限増 80/100 実験)の効果測定で「上限を上げても denial が残れば B 経路で自発放棄が続く」という誤解が生じる。
...
| `Auto-Update-Triggered` | SM-10 の自動回復で `out-of-date with base branch` を検知し `PUT /pulls/{number}/update-branch` 成功、本 run は `gh pr merge --auto` を skip & exit(新 SHA で CI 再走 → re-review → 新 run で `Auto-Merge-Enabled` に乗る) | 新 SHA workflow run の起動を待つ |
| `Auto-Update-Failed` | SM-10 の `update-branch` 呼出が HTTP 4xx/5xx で失敗 | `needs-human-decision` 付与 + `@win2cot` メンション + 人手介入 |
...
#### reason enum(state 別の補助情報)

`reason` は state に対する補助情報。state ごとに使用する enum 集合が異なる。

**`Impl-Aborted` / `Impl-Fix-Aborted` / `Review-Aborted` の場合**(Aborted 系の reason):

- `max-turns` — claude-code-action の `error_max_turns` で停止(SDK 出力で確認。**A 経路**: §2.1.3 のソフト締切仕様で tool_use ループ継続中に強制終了したケース)
...
- `cancelled` — `${{ steps.claude.outcome }} == cancelled`(workflow cancellation 経由の停止)
- `denial-loop` — allowedTools 不足等で claude が必要 tool を denied され続け、`signal: claude-impl-blocked` を投げる前に自発的に wrap-up 完了モードに入った場合(B 経路、`subtype: success` + `permission_denials_count` 高 + `artifacts.branch_created: false` で識別。観察源: tasks-webapi #142 / run 26410452890、17 denials で無言終了)
- `unknown` — 上記いずれにも分類できない

**A 経路と B 経路の地続き 2 状態について(SM-21)**:

`Impl-Aborted(reason=max-turns)` の A 経路(`subtype: error_max_turns`)と、`subtype: success` 無言終了の B 経路は **連続した 2 状態** である。どちらも「claude が実質未完成で終了した」ケースだが、SDK が返す `subtype` が異なる:

- **A 経路**: num_turns が N を超えた時点で tool_use ループ継続中 → SDK が強制終了 → `subtype: error_max_turns` → `Impl-Aborted(reason=max-turns)`
- **B 経路**: num_turns が N を超えた時点で wrap-up モード → `subtype: success / is_error: false` で着地するが branch 未生成 + signal なし → SM-20 で `reason=denial-loop` として識別 → cleanup 側で `Impl-Aborted` 扱いに補正

state 判定では `subtype` だけでなく `artifacts`(branch_created 等)との組合せで判断する(§2.7 補足参照)。

**`Impl-Done` / `Impl-Fix-Done` の場合**(Done 系の reason、2026-05-28 SM-14c 追加):

- `signal-missing` — claude が PR を作成完遂(impl)または PR head に新 commit push(impl-fix)したが完了 signal コメント投稿を省略して終了。detect-state が PR 実態から Done と判定(false positive 抑止経路、SM-14c)。通常の Done(signal あり)では `reason` 空

reason の特定方法は claude-code-action の SDK 出力ログ + GHA step の outcome + PR 実態(headRefOid / isDraft / state)を組合せて判定する。詳細は `docs/automation/conventions.md` に書き下す。
...
Impl-In-Progress --subtype:success 無言終了 (B 経路, SM-20)--> Impl-Aborted(reason=denial-loop)

Review-Pending --review 起動--> Review-In-Progress
Review-In-Progress --approve--> Review-Approved
Review-Approved --mergeable_state=behind 検知 + update-branch 成功 (SM-10)--> Auto-Update-Triggered --新 SHA で CI 再走 + re-review APPROVED--> Review-Approved (new run)
Review-Approved --mergeable_state=behind 検知 + update-branch 失敗 (SM-10)--> Auto-Update-Failed
Review-Approved --behind でない or fail-open--> Auto-Merge-Enabled --CI green + up-to-date--> Merged
Review-Approved --auto-merge enable 失敗--> Auto-Merge-Failed
...
| 不正遷移 | 抑止する派生実装 | 抑止方法 |
|---|---|---|
| `field ≠ 適用` の Issue で `claude:ready` 起動 | SM-13 | impl workflow 冒頭で Project field を GraphQL 取得、`適用` 以外なら早期 exit + キャンセル理由コメント + `claude:ready` 自動除去。**※ 2026-05-27 ROI 再評価で v1.2.0 では実装保留。理由: Project field の GraphQL 読み取りには user-level `Projects: Read` permission が必要で、SM-19 で凍結した case A(impl App permission 拡張)領域に逆戻りする。代替案(専用 PAT / 別 App / label mirroring)はいずれも追加 setup or 二重情報源で ROI に見合わず。「`claude:ready` 誤付与による意図しない impl 起動」の実害観察もゼロ。機能要件は本表に維持、Sprint 1(2026-07-14 開始)後に実害観測 or Projects 読み取り経路解決時点で復活判断。§7 残課題参照** |
| `Impl-In-Progress` → `subtype: success` 無言終了(branch / PR 未生成、signal なし) | SM-20 + SM-14 | impl Claude 側で「必要 tool が denied され続ける場合は `signal: claude-impl-blocked` を投げて停止」を `.github/claude/system-impl.md` カテゴリ B に明文化(SM-20、カテゴリ B の対象を「レビュー指摘に対応しようとして」から「実装または修正に着手しようとして」に拡大 + 初期 impl で必要 tool が denied され続けた場合の例を追加)。cleanup 側は `artifacts.branch_created: false` を検知して `Impl-Aborted(reason=denial-loop)` 扱いに揃える(SM-14、ADR §2.7 detect-state ロジックは SM-14c #78 で既に該当判定を含む)。観察源: tasks-webapi #142 / run 26410452890 |
| `Impl-Aborted` 後の宙ぶらりん | SM-14 | impl / impl-fix / review の最終 step を `if: always()` で実行。**detect-state の Aborted 判定は SM-14c (#78) で精緻化**: impl は signal 不在 AND (PR 不在 OR draft) を Aborted、signal 不在 AND PR 完遂 (non-draft, open/merged) は `Impl-Done` + `reason=signal-missing` として記録(false positive 抑止)。impl-fix は signal 不在 AND PR head SHA 変化なし or draft を Aborted、signal 不在 AND 新 commit + non-draft は `Impl-Fix-Done` + `reason=signal-missing`。Aborted 判定時のみ着手宣言コメントに「max-turns 等で異常終了、未着手」を追記 + `claude:ready` 除去 + `needs-human-decision` 付与 |
| 古い `Review-RequestedChanges` 残置 | SM-08 | impl-fix の `signal: claude-impl-done` 投稿時(state == `Impl-Fix-Done`)に、PR 上の active な `CHANGES_REQUESTED` review(reviewer ごとの最新 review が `CHANGES_REQUESTED` のもの)を全件 `gh api -X PUT /repos/{owner}/{repo}/pulls/{pull_number}/reviews/{review_id}/dismissals` で自動 dismiss(reviewer 不問)|
| 人手 fix 後の review 再起動経路欠落 | SM-01 | `<sub>signal: claude-human-fix-done</sub>` を `author_association ∈ {OWNER, MEMBER, COLLABORATOR}` 限定で issue_comment trigger 受付 → review 起動(reusable-review.yml caller の `if:` 条件に第 3 分岐として追加、2026-05-28 採用、案 (i))。検討した代替: **案 (ii) `re-review` ラベル付与 trigger** = label を毎回付け直す運用負担が高く、付与権限の制限も repo の write 権限管理と二重メンテになるため却下。**案 (iii) Draft↔Ready 切替の運用ドキュメント化のみ** = 本 ADR §1 の「ステートマシン正本化」方針と整合せず、人手 PR の re-review 経路がコードで保証されないため却下 |
| `Review-Approved` だが `out-of-date with base branch` | SM-10 | auto-merge enable 直前に `gh api /repos/{owner}/{repo}/pulls/{number} --jq .mergeable_state` を取得、`unknown` を引いた場合のみ 2 秒 sleep して 1 回 retry。`behind` のみ自動回復対象、それ以外(`clean` / `blocked` / `dirty` / retry 後も `unknown`)は fail-open で enable に進む。`behind` 検知時は `gh api -X PUT /repos/{owner}/{repo}/pulls/{number}/update-branch` を呼出し、成功なら本 run は state=`Auto-Update-Triggered` で記録して `gh pr merge --auto` を skip & exit(新 SHA で `pull_request.synchronize` → CI 再走 → re-review → 新 run で auto-merge enable に乗る、§2.5 自己回復と整合)。失敗(HTTP 4xx/5xx)時は state=`Auto-Update-Failed` + `needs-human-decision` 付与 + `@win2cot` メンション付きコメント |
...
| SM-02 impl-fix の checkout 認証エラー(人手 PR を impl-fix が拾った時の token scope mismatch) | 前提条件 2.1.1「impl-fix の checkout SHA」が崩れる事象として Tier 1 明文化 + 派生実装で対応。`actions/checkout` の `token` 引数を `claude-automation-impl` App token に明示し、人手 PR でも fallback できる経路を確保。**※ 2026-05-29 ROI 再評価で v1.2.0 では実装保留。理由: 起票根拠の tasks-webapi PR #205 期(2026-05-23)観察以降、SM-05 (#81 actions/checkout@v4→@v5 一括) / SM-01 (#73 人手 PR の re-review 経路を signal 方式に限定) / SM-19 (#63) / SM-25 (#65) / SM-04 (#80) の merge で起票時の前提が消滅した可能性が高く、再観測記録なし。SM-L1 / SM-13 と同じく「未観測の障害カテゴリへの予防装置」相当。機能要件は本表に維持、Sprint 1(2026-07-14 開始)で実 load 観察後に復活判断。§7 残課題参照** |

#### Tier 2(ADR で想定挙動だけ明文化、実装は当面しない)

未観測だが理論上発生しうる事象。観測されてから実装に格上げする。

**`Impl-Done` と `subtype: success` 無言終了は別状態(SM-21)**:

`Impl-Done`(branch 作成 + PR ready_for_review + `signal: claude-impl-done`)と `subtype: success` 無言終了(branch 未生成 + signal なし)は **異なる状態** であり、detect-state ロジックでは両者を区別する必要がある:

- `Impl-Done`: `subtype: success` かつ `artifacts.branch_created: true` かつ PR non-draft かつ signal あり
- `subtype: success` 無言終了(B 経路): `subtype: success` かつ `artifacts.branch_created: false` / signal なし → SM-20 で `reason=denial-loop` として識別 → `Impl-Aborted` 扱いで cleanup

SM-20(障壁検知カテゴリ B 拡張)と連動する(Issue の背景参照)。
...
- 自動回復試行(失敗時のみ通知)
- 自動回復試行(失敗時も通知なし、workflow log のみ)
- 即時通知
- 静かにログ
- 不要
...
| field 不整合での `claude:ready` 起動キャンセル(SM-13) | 即時通知 | 即時メンション | 本体 Issue(※ SM-13 は 2026-05-27 v1.2.0 不採用 / 保留、§2.4 表注釈・§7 残課題参照) |
...
| SM-10 auto-update branch の自動回復 | 自動回復試行 → 失敗時のみ通知 | 失敗時メンション | 本体 Issue/PR + 実行ログ収集 |
| SM-08 requested_changes dismiss の自動回復 | 自動回復試行(失敗時も通知なし、workflow log のみ) | (メンションなし) | workflow log のみ(`::warning::`、runs-log JSON には state のみ反映、re-review で上書きされるため非クリティカル)|
| SM-16 / SM-17 audit GHA の検知結果 | バッチ集約 | バッチ集約(重大な不整合のみメンション付き) | audit-log(※ SM-16 / SM-17 は 2026-05-29 v1.2.0 不採用 / 保留、SM-02 連動。§2.8 ADR 外独立 list 注釈・§7 残課題参照)|
...
|---|---|---|
| **runs-log Issue** ([#57](https://github.com/win2cot/claude-automation/issues/57)) | 特定 Target に対して作業した workflow の実行結果 | impl / impl-fix / review / auto-merge / notify-human |
| **runs-log-test Issue** ([#58](https://github.com/win2cot/claude-automation/issues/58)) | composite action `runs-log-append` の単体検証専用(`test-runs-log-append.yml` 経由、`workflow_dispatch`)| (本番 workflow からは投稿しない) |
| **audit-log Issue** ([#59](https://github.com/win2cot/claude-automation/issues/59)) | 複数 Target を scan / audit する workflow の検知結果 | SM-L1 起動失敗 watchdog / SM-16 field audit / SM-17 反映漏れチェック(※ SM-L1 / SM-16 / SM-17 はいずれも v1.2.0 不採用 / 保留、Sprint 1 後再評価。Issue 自体は将来復活時の receiver として open 維持) |
...
audit GHA 用(SM-L1 起動失敗 watchdog / SM-16 field audit / SM-17 反映漏れチェック、いずれも v1.2.0 不採用 / 保留中):
...
- `state` — workflow に応じて以下のいずれか:
  ...
  **判定注意(SM-21)**: `state` の判定では **`subtype` と `artifacts` の両方** を参照する。`subtype: success` だけでは `Impl-Done` と確定できない。`subtype: success` かつ `artifacts.branch_created: false` / signal なし の場合は B 経路の自発的放棄(§2.3 A/B 経路補足参照)であり、`Impl-Aborted` として記録する。
...
- **SM-13 claude:ready 空振り** — impl workflow 冒頭で Project field を取得し `適用` 以外なら早期 exit(**2026-05-27 v1.2.0 不採用 / 保留、Sprint 1 後再評価。詳細は §2.4 表注釈・§7 残課題参照**)
- **SM-08 requested_changes 自動 dismiss** — impl-fix の `signal: claude-impl-done` 投稿時に古い requested_changes を dismiss
- **SM-01 人力修正の再レビュー経路** — `<sub>signal: claude-human-fix-done</sub>` を `author_association ∈ {OWNER, MEMBER, COLLABORATOR}` 限定で issue_comment trigger 受付(2026-05-28 採用、案 (i))。代替案 (ii) `re-review` label trigger / (iii) doc only は §2.4 表に不採用理由を記載
...
- **SM-02 impl-fix checkout 認証エラー対応** — 前提条件を崩す事象として ADR 内 Tier 1 で扱う。`actions/checkout` の `token` 引数 + 人手 PR 拾い時の fallback 設計(**2026-05-29 v1.2.0 不採用 / 保留、Sprint 1 後再評価。詳細は §2.4 表注釈・§7 残課題参照**)
...
- **SM-16 field audit 防波堤** — Project Custom Field 空欄スキャン、優先度最低、Step 13 完了時に再判定(**2026-05-29 v1.2.0 不採用 / 保留、SM-02 連動。Sprint 1 後再評価。詳細は §7 残課題参照**)
- **SM-17 反映漏れ防波堤** — ADR / 設計規約 merge 時の関連 Issue 追従チェック、優先度最低(**2026-05-29 v1.2.0 不採用 / 保留、SM-02 連動。Sprint 1 後再評価。詳細は §7 残課題参照**)
...
| SM-02 | (b) | impl-fix checkout 認証エラー対応 | Tier 1 派生(前提条件 §2.1.1 を崩す事象として明文化、2026-05-29 v1.2.0 不採用 / 保留、§2.4 表注釈参照)|
...
| SM-12 | (l) | 半適用 復活 | 不採用 |
| SM-13 | (m) | claude:ready 空振り抑止 | Tier 1 派生(2026-05-27 v1.2.0 不採用 / 保留、§2.4 表注釈参照)|
| SM-14 | (n) | max-turns 後処理(cleanup) | Tier 1 派生 / SM-14c (#78) follow-up あり(detect-state を signal 不在 AND PR 不在/draft の AND 条件に精緻化、reason 列に `signal-missing` enum 追加、2026-05-28 議論完了 = tracker #36 追記その 9 参照)|
| SM-15 | (p) | 実行ログ収集の収集側実装 | ADR 外 / 独立 PR(record スキーマは ADR §2.7) |
| SM-16 | (q) | field audit 防波堤 GHA | ADR 外 / 独立 PR(優先度最低、2026-05-29 v1.2.0 不採用 / 保留、SM-02 連動、§2.4 表注釈参照)|
| SM-17 | (r) | 反映漏れ防波堤 GHA | ADR 外 / 独立 PR(優先度最低、2026-05-29 v1.2.0 不採用 / 保留、SM-02 連動、§2.4 表注釈参照)|
...
- **新規 GHA**: SM-L1 watchdog GHA、SM-15 実行ログ収集 step の追加、SM-16 / SM-17 防波堤 GHA(後者は優先度最低)。**※ SM-L1 / SM-16 / SM-17 はいずれも v1.2.0 不採用 / 保留、Sprint 1 後再評価。SM-15 のみ v1.2.0 で完走済(#37 / PR #60-62)**
...
- **SM-13 field 不整合**: テスト用 Issue を「不適用」field で起票 → `claude:ready` 付与 → workflow 早期 exit + キャンセル理由コメント + ラベル除去を確認(**v1.2.0 では未着手、SM-13 復活 PR 時にこの検証手順を実施**)
- **SM-08 requested_changes dismiss**: ローカル sample app PR で意図的に requested_changes → impl-fix → 古い review が dismiss されることを確認
- **SM-10 auto-update branch**: lab(`test-auto-merge.yml`)で意図的に out-of-date な PR を作って approve、本 run が state=`Auto-Update-Triggered` で exit、新 SHA で CI 再走 → review-claude 再走 → 新 run で auto-merge enable → merge まで進むことを確認。`mergeable_state: unknown` の retry 1 回 fail-open / `behind` 以外のスキップも併せて確認
...
- **SM-02 impl-fix checkout**: 人手 PR で意図的に impl-fix を起動 → token scope が正しく動作するか確認(**v1.2.0 では未着手、SM-02 復活 PR 時にこの検証手順を実施**)
...
- runs-log Issue [#57](https://github.com/win2cot/claude-automation/issues/57)(append-only, never close)— SM-15 で open
- runs-log-test Issue [#58](https://github.com/win2cot/claude-automation/issues/58)(`test-runs-log-append.yml` 単体検証用)— SM-15 で open
- audit-log Issue [#59](https://github.com/win2cot/claude-automation/issues/59)(append-only, never close)— SM-15 で open(SM-L1 / SM-16 / SM-17 で使用予定、いずれも v1.2.0 不採用 / 保留、Sprint 1 後再評価。Issue 自体は将来復活時の receiver として open 維持)
- `docs/automation/conventions.md` §8 — runs-log / audit-log フォーマット書き下し(SM-15)
- `.github/actions/runs-log-append/action.yml` / `audit-log-append/action.yml` — composite action(SM-15)
...
- ~~**SM-16 / SM-17 防波堤 GHA の着手要否最終判定**: Step 13 完了時に Cowork proactive fill の実運用穴を再評価して着手要否を決める。v1.3.0 繰越も選択肢~~(**2026-05-29 解消: SM-02 保留に連動して SM-16 / SM-17 も保留決定。本セクション末尾の SM-16 / SM-17 復活判断 entry に再評価条件を明記**)
...
- **SM-02 impl-fix checkout 認証エラー対応の復活判断**: 2026-05-29 ROI 再評価で v1.2.0 不採用 / 保留。再評価条件 = Sprint 1(2026-07-14 開始)以降の実 load 観察で「impl-fix の checkout 認証エラー」が 1 件以上再観測される、または `actions/checkout` v5 系で同種事象が報告される。判断時の参照 Issue は #45(保留 close 済み)、復活時は新規 Issue として再起票
- **SM-16 field audit 防波堤 GHA の復活判断**: 2026-05-29 ROI 再評価で v1.2.0 不採用 / 保留(SM-02 連動、tracker #36 完了条件で「SM-02 完了時に着手要否再判定」とされていたため、SM-02 保留に連動して再判定起点が消滅)。再評価条件 = (a) Sprint 1 以降の実 load 観察で「Project Custom Field 空欄による automation 起動失敗」が 1 件以上観測される、または (b) SM-02 復活時に同時着手判定。判断時の参照 Issue は #50(保留 close 済み)、復活時は新規 Issue として再起票
- **SM-17 反映漏れ防波堤 GHA の復活判断**: 2026-05-29 ROI 再評価で v1.2.0 不採用 / 保留(SM-02 連動、tracker #36 完了条件で「SM-02 完了時に着手要否再判定」とされていたため、SM-02 保留に連動して再判定起点が消滅)。再評価条件 = (a) Sprint 1 以降の実 load 観察で「ADR / 設計規約 merge 時の Issue 追従漏れ」が 1 件以上観測される、または (b) SM-02 復活時に同時着手判定。判断時の参照 Issue は #51(保留 close 済み)、復活時は新規 Issue として再起票
