...
実装 Claude は修正コミット push 後、**同一ジョブで** PR コメントに「対応完了レポート」を投稿する。コメント本文には必ず以下のマーカーをフッターに含める。

```html
<sub>signal: claude-impl-done</sub>
```

このマーカーをもって `reusable-review.yml` の発火条件(`contains(comment.body, 'signal: claude-impl-done')`)とする。push 単独では再レビューは起動しない。
...
`!` を含まない `<sub>signal: claude-impl-done</sub>` 形式は:
- bash 履歴展開でエスケープされない
