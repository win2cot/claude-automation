...
- [ ] スコープ逸脱なし
- [ ] 日本語ドキュメント更新(必要時)
- [ ] CI 結果(in_progress/queued/pending は ⚠・approve をブロックしない。concluded=failure のみ ✗。詳細は後述「CI の扱い」)
- [ ] 未対応テーブル空
...
1. 観点チェックリスト全項目 ✓(CI 結果は「CI の扱い」に従い、進行中の ⚠ は approve を妨げない)
2. PR description の未対応テーブルに `open` 状態の指摘なし
3. CI が concluded=failure でないこと(既存 CI のみ判定対象、Claude workflow は含めない)。in_progress/queued/pending は approve をブロックしない
4. 設計分岐ではない
...
CI green の **最終担保は GitHub branch protection + auto-merge** が行う(reviewer が green を保証する必要はない。approve 後も merge は CI green が揃うまで GitHub 側で待機する)。reviewer は CI の状態を次のように扱う:

- **in_progress / queued / pending(まだ走っている)**: ⚠ で「CI 進行中」とだけ出力し、**approve をブロックしない / changes_requested の根拠にしない**。reviewer 起動時点(PR open / `signal: claude-impl-done`)では CI(特に NativeBuild 等の長時間ジョブ)が未完了なのが通常であり、未完了を「green でない」と扱って changes_requested を出してはならない。
- **concluded = success(green)**: ✓。
- **concluded = failure / cancelled / timed_out**: ✗ とし、対象ジョブ名を明記して changes_requested(実装 Claude が修正に入る)。

つまり CI を理由に changes_requested を出すのは **確定的に失敗したときのみ**。進行中・未完了を理由には出さない。コード自体に指摘が無く CI が進行中なだけなら approve してよい。
